...
if you'd like to use original scrumblr go to [scrumblr.ca](http://scrumblr.ca). new boards are made simply by modifying the url to something unique. e.g. your team could use a shared board at: *http://scrumblr.ca/thisisoursecretboard23423242*

you can follow the instructions below to setup this fork yourself. it is very simple -- it just uses node.js and mongodb/redis (optional).
...
scrumblr works on up to date chrome and firefox browsers. enable websockets for optimal performance. this was not designed for browser support. use chrome for this app.
...
- Select data storage: [install mongodb](https://docs.mongodb.com/manual/administration/install-community/) (tested v4.0.2) or [install redis](http://redis.io/download) (last tested on v2.8.4) or plain JSON files
- [install node.js](http://nodejs.org/) (last tested on v8.11.4)
