...
db.prototype = {
	...
	setRevisions: function(room, revisions) {
		fs.writeFile('/cihwrk/ECPSScrumblr/save-data' + room + '/revisions.json', JSON.stringify(revisions), function(err) {
			if(err) {
				console.log(err);
			}
		});
	...
	getRevisions: function(room, callback) {
		fs.readFile('/cihwrk/ECPSScrumblr/save-data' + room + '/revisions.json', 'utf8', function (err, data) {
			if (err) {
				callback(null);
				return console.log('No revisions.json file found to load for the room ' + room);
			}
