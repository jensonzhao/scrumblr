...
db.prototype = {
	...
	setFont: function(room, font) {
		redisClient.set(REDIS_PREFIX + '-room:' + room + '-font', JSON.stringify(font));
	},

	getFont: function(room, callback) {
		redisClient.get(REDIS_PREFIX + '-room:' + room + '-font', function (err, res) {
	...
	setPassword: function(room, password) {
		redisClient.set(REDIS_PREFIX + '-room:' + room + '-password', password);
	},

	getPassword: function(room, callback) {
		redisClient.get(REDIS_PREFIX + '-room:' + room + '-password', function (err, res) {
			if(res) {
				callback(res);
			} else {
				callback(null);
	...
	{
		redisClient.set(REDIS_PREFIX + '-room:' + room + '-password', null);
